...
| insn :: prg' ->
  (match insn with
    | BINOP op ->
      ...
      eval env (cstack, (Value.of_int (Expr.binOpEval op (Value.to_int x) (Value.to_int y))) :: stack', c) prg'
    | CONST i ->
      eval env (cstack, (Value.of_int i) :: stack, c) prg'
    | STRING s ->
      eval env (cstack, (Value.of_string s) :: stack, c) prg'
    | SEXP (s, n) ->
      let v, newStack = split n stack in
      eval env (cstack, (Value.sexp s (List.rev v)) :: newStack, c) prg'
    | LD x ->
      eval env (cstack, State.eval st x :: stack, c) prg'
    | ST x ->
      let z :: stack' = stack in
      eval env (cstack, stack', (State.update x z st, i, o)) prg'
    | STA (x, n) ->
      let v :: ii, stack' = split (n + 1) stack in
      eval env (cstack, stack', (Language.Stmt.update st x v (List.rev ii), i, o)) prg'
    | LABEL s ->
      eval env conf prg'
    | JMP name ->
      eval env conf (env#labeled name)
    | CJMP (condition, name) ->
      eval env (cstack, tl stack, c) (if (checkConditionalJump condition (hd stack)) then (env#labeled name) else prg')
    | CALL (f, n, p) ->
      if env#is_label f
        then eval env ((prg', st)::cstack, stack, c) (env#labeled f)
        else eval env (env#builtin conf f n p) prg'
    | BEGIN (_, args, locals) ->
      let resolvedArgumentsMapping, newStack = resolveArgumentsMapping [] args stack in
      eval env (cstack, newStack, (List.fold_left (fun s (x, v) -> State.update x v s) (State.enter st (args @ locals)) resolvedArgumentsMapping, i, o)) prg'
    | END | RET _ ->
      (match cstack with
      | (prg', st')::cstack' -> eval env (cstack', stack, (State.leave st st', i, o)) prg'
      | [] -> conf)
    | ENTER xs ->
      let vals, st1 = split (List.length xs) stack in
      let combined = List.combine xs vals in
      eval env (cstack, st1, (State.push st (List.fold_left (fun s (x, v) -> State.bind x v s) State.undefined combined) xs, i, o)) prg'
    | LEAVE ->
      eval env (cstack, stack, (State.drop st, i, o)) prg'
    | DROP ->
      eval env (cstack, List.tl stack, c) prg'
    | DUP ->
      eval env (cstack, List.hd stack :: stack, c) prg'
    | SWAP ->
      let fst :: snd :: st1 = stack in
      eval env (cstack, snd :: fst :: st1, c) prg'
    | TAG t ->
      let fst :: st1 = stack in
      eval env (cstack, (Value.of_int (match fst with Value.Sexp (t', _) when t' = t -> 1 | _ -> 0)) :: st1, c) prg'
  )
