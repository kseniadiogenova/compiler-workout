...
| insn :: prg' ->
  (match insn with
    | BINOP op ->
      ...
      eval env (cstack, Expr.to_func op x y :: stack', c) prg'
    | READ ->
      let z :: i' = i in
      eval env (cstack, z::stack, (st, i', o)) prg'
    | WRITE ->
      let z :: stack' = stack in
      eval env (cstack, stack', (st, i, o @ [z])) prg'
    | CONST i ->
      eval env (cstack, i :: stack, c) prg'
    | LD x ->
    ...
    | CJMP (condition, name) ->
      eval env conf (if (checkConditionalJump condition (hd stack)) then (env#labeled name) else prg')
    | CALL (f, _, _) ->
      eval env ((prg', st)::cstack, stack, c) (env#labeled f)
    | BEGIN (_, args, locals) ->
...
let rec compileWithLabels labels =
  ...
  | Expr.Call (f, args) ->
    compileCall f args and compileCall f args =
    let compiledArgsList = List.map expr (List.rev args) in
    let compiledArgs = List.concat compiledArgsList in
    compiledArgs @ [CALL (funcLabel f, List.length args, false)] in
    function
    | Stmt.Seq (s1, s2) ->
    ...
    | Stmt.Call (f, args) ->
      labels, compileCall f args
    | Stmt.Return res ->
      labels, (match res with None -> [RET false] | Some v -> expr v) @ [RET true]

let compileFuncDefinition labels (name, (args, locals, body)) =
  let endLabel, labels1 = labels#new_label in
...
let compileAllFuncDefinitions labels defs = 
  List.fold_left (fun (labels, allDefsCode) (name, others) ->
  let labels1, singleCode = compileFuncDefinition labels (funcLabel name, others) in labels1, singleCode::allDefsCode) (labels, []) defs
...
*)
let compile (defs, p) =
  let endLabel, labels = (new labels)#new_label in
