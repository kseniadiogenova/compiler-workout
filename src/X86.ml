              ...
              let env, code =
                if n = 0
                  then
                    ...
                    | 0 -> env, acc
                    | n -> let x, env = env#pop in push_args env ((Push x)::acc) (n-1) in
...
let fixedInit n f = 
  let rec fixedInit' i n f = if i >= n
    then []
    else (f i) :: (fixedInit' (i + 1) n f)
  in fixedInit' 0 n f

(* Environment implementation *)
let make_assoc l = List.combine l (fixedInit (List.length l) (fun x -> x))
