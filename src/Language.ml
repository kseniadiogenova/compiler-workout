...
module Value =
  struct
    ...
    let update_array  a i x = l_init   (List.length a)   (fun j -> if j = i then x else List.nth a j)
...
module Expr =
  struct
    ...
    *)                                                       
    ...
    let rec eval env ((st, i, o, r) as conf) expr = match expr with
      ...
      | Binop (op, exp1, exp2) -> let (_, _, _, Some firstArg) as conf = eval env conf exp1 in
        ...
        in (st, i, o, Some (Value.of_int (binOpEval op (Value.to_int firstArg) (Value.to_int secondArg)))) 
    ...
    *)
    ostap (                                      
      parse:
        !(Ostap.Util.expr(fun x -> x) (Array.map (fun (a, s) ->
          a, List.map
            (fun s ->
              ostap(- $(s)),
            (fun x y ->
      ...
      primary:
        b:base is:(-"[" i:parse -"]" {`Elem i} | "." %"length" {`Len}) * {List.fold_left (fun x -> function `Elem i -> Elem (x, i) | `Len -> Length x) b is};
      base:
        n:DECIMAL {Const n}
  ...
  struct
    ...
    *)
    ...
    let rec eval env ((st, i, o, r) as conf) k stmt = 
      match stmt with
      | Assign (x, ii, e) -> 
        let (st, i, o, ii) = Expr.eval_list env conf ii in
        let (st, i, o, Some v) = Expr.eval env (st, i, o, None) e in
        eval env (update st x v ii, i, o, None) Skip k
      | Seq (s1, s2) ->
      ...
      | If (expr, thenIf, elseIf) ->
        ...
        if Value.to_int x <> 0 then (eval env conf k thenIf) else (eval env conf k elseIf)
      | While (expr, loopStmt) ->
        let (_, _, _, Some x) as conf = Expr.eval env conf expr in
      ...
      | Case (expr, branches) ->
        let (_, _, _, Some evaluatedExpr) as newConf = Expr.eval env conf expr in
        let rec findCorrectBranch ((st, i, o, _) as conf) branchess =
          let (patternMatching, body)::tailBranches = branchess in
        let rec matchPattern pattern value st =
          let update x v = function
            | None ->
              None 
            | Some s ->
              Some (State.bind x v s)
          in
          match pattern, value with
            | Pattern.Wildcard, _ ->
              st
            | Pattern.Sexp (s, xs), Value.Sexp (s', xs') when s = s' ->
              matchList xs xs' st
            | Pattern.Ident x, v ->
              update x v st  
            | _ ->
        ...
        and
          matchList xs xs' s = match xs, xs' with
            | [], [] ->
              s
            | x::xs, x'::xs' ->
              matchList xs xs' (matchPattern x x' s)
            | _ ->
              None
          in 
            match matchPattern patternMatching evaluatedExpr (Some State.undefined) with
              | None ->
                findCorrectBranch conf tailBranches
              | Some st' ->
                eval env (State.push st st' (Pattern.vars patternMatching), i, o, None) k (Seq (body, Leave))
        in
          findCorrectBranch newConf branches
      | Leave ->
        eval env (State.drop st, i, o, r) Skip k
      | Return res ->
        match res with
        | None ->
          (st, i, o, None)
    ...
    ostap (
      ...
      stmt:
        x:IDENT
          assignmentOrCall: (
            ii:(-"[" !(Expr.parse) -"]")* ":=" e:!(Expr.parse) {Assign (x, ii, e)}
